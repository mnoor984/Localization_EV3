...
public class Odometer extends Thread implements Runnable {
  ...
  int lastTachoL = 0;               //lecture
  int lastTachoR = 0;               //lecture
  ...
  private Odometer() {
    setXyt(0, 0, 0);
    lastTachoL = leftMotor.getTachoCount();                 // added by us
    lastTachoR = rightMotor.getTachoCount();                // added by us
  ...
  public void run() {
    ...
    long updateStart;
    long updateDuration;
    double tempTheta;           //NEW
    tempTheta = odo.getXyt()[2] * (Math.PI/180);       //NEW
    while (true) {
    ...
    if (updateDuration < ODOMETER_PERIOD) {
      sleepFor(ODOMETER_PERIOD - updateDuration);               /** SLEEEP **/
      }
    }           //end of while loop
  } // end of run method
